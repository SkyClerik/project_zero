...
{
    public class KeyItemDetailPanel : VisualElement
    {
        private Action _onDefinitionNameChangedCallback;
        private KeyItemDefinition _currentDefinition;
        ...
        public KeyItemDetailPanel(Action onDefinitionNameChangedCallback)
        {
        ...
        {
            Clear();
            _currentDefinition = selected as KeyItemDefinition; // Cast to specific type
            ...
            {
                Add(new Label("Выберите ключевой предмет из списка для редактирования.")
                {
            ...
            if (hasVisibleProperties)
            {
                do
                {
                    ...
                    {
                        // Специальная логика для KeyItemDetailPanel
                        // if (_currentDefinition is KeyItemDefinition && propertyIterator.name == "SomeSpecificField")
                        // {
